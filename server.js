...
const JWT_SECRET = 'krista-super-secret-key-2026-замените';
...
function readData() {
    try {
        ...
        // Если файл не существует или повреждён — создаём начальную структуру
        const initialData = {
            users: [],
            chats: [],
            messages: [],
...
// Поиск пользователя по токену (мидлвар)
...
// Храним подключения: { iun: WebSocket }
...
wss.on('connection', (ws, req) => {
        ...
        try {
            ...
            if (type === 'auth') {
                ...
                try {
                    ...
                    console.log(`✅ Пользователь ${ws.iun} авторизован в WebSocket`);
                    
                    // Оповещаем всех о статусе "в сети"
                    broadcast({
                        type: 'status',
                        payload: { iun: ws.iun, status: 'online' }
                    });
                } catch (err) {
            ...
            if (type === 'newMessage') {
                ...
                if (!chat) {
                    ws.send(JSON.stringify({ type: 'error', payload: 'Чат не найден' }));
                    return;
                }
                ...
                if (chat) {
                    chat.members.forEach(iun => {
                        ...
                        if (client && client.readyState === WebSocket.OPEN) {
                            client.send(JSON.stringify({
                                type: 'reaction',
                                payload: { messageId, reaction, user, removed: idx > -1 }
                            }));
                        }
                    });
                }
    ...
    ws.on('close', () => {
        if (ws.iun) {
            ...
            // Оповещаем о статусе "не в сети"
            broadcast({
                type: 'status',
                payload: { iun: ws.iun, status: 'offline' }
            });
...
// Функция широковещательной рассылки всем
...
app.get('/api/chats', authMiddleware, (req, res) => {
    ...
    // Получаем чаты, где пользователь участник и не архивирован
    let chats = data.chats.filter(c => 
        c.members.includes(req.userIun) && !c.isArchived
    );

    // Добавляем последнее сообщение и непрочитанные
    chats = chats.map(chat => {
        const messages = data.messages.filter(m => m.chatId === chat.id && !m.deleted);
        const lastMsg = messages.length > 0 ? messages[messages.length - 1] : null;
        const unreadCount = messages.filter(m => 
            m.sender !== req.userIun && new Date(m.timestamp) > new Date(user.lastSeen)
        ).length;
    ...
    // Сортируем: закреплённые вверху, затем по времени обновления
...
app.post('/api/chats', authMiddleware, (req, res) => {
    ...
    if (!type || !['personal', 'group', 'channel'].includes(type)) {
    ...
    if (type === 'personal') {
        const existing = data.chats.find(c => 
            c.type === 'personal' &&
            c.members.includes(req.userIun) &&
            c.members.includes(members[0])
        );
        if (existing) {
            return res.status(409).json({ error: 'Чат уже существует', chatId: existing.id });
        }
    }
    ...
    // Оповещаем всех участников о новом чате
...
app.get('/api/chats/:chatId/messages', authMiddleware, (req, res) => {
    ...
    if (!chat.members.includes(req.userIun)) {
...
app.post('/api/chats/:chatId/pin', authMiddleware, (req, res) => {
    ...
    if (idx > -1) {
        ...
        if (user.pinnedChats.length >= 10) {
            return res.status(400).json({ error: 'Лимит 10 закреплённых чатов' });
        }
...
app.get('/api/feed', authMiddleware, (req, res) => {
    ...
    const channels = data.chats.filter(c => 
        c.type === 'channel' && 
        c.members.includes(req.userIun) &&
        !c.isArchived
    );
...
app.post('/api/import/config', authMiddleware, (req, res) => {
    ...
    if (config.pinnedChats && Array.isArray(config.pinnedChats)) {
        user.pinnedChats = config.pinnedChats.slice(0, 10);
    }
    if (config.blocked && Array.isArray(config.blocked)) {
        user.blocked = config.blocked;
    }
...
app.delete('/api/admin/chats/:iun', authMiddleware, (req, res) => {
    ...
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        if (!decoded.god) {
            return res.status(403).json({ error: 'Требуется режим бога' });
        }
    } catch {
        return res.status(401).json({ error: 'Неверный токен' });
    }
...
app.post('/api/block', authMiddleware, (req, res) => {
    ...
    if (!user.blocked.includes(iun)) {
        user.blocked.push(iun);
        writeData(data);
    }
