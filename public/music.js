...
(function(){
    ...
    if (!el.innerHTML.trim()) {
      el.innerHTML = `
        <div style="display:flex;gap:4px;padding:8px 10px;overflow-x:auto;flex-shrink:0;border-bottom:1px solid var(--glass-border);background:var(--glass);scrollbar-width:none" id="musicTabs">
          ...
          <div style="flex-shrink:0;background:var(--glass-2);border:1px solid var(--glass-border);color:var(--text2);padding:7px 14px;border-radius:14px;font-family:var(--font-body);font-size:12px;font-weight:500;cursor:pointer;white-space:nowrap" class="music-tab active" data-t="songs">Песни</div>
          <div style="flex-shrink:0;background:var(--glass-2);border:1px solid var(--glass-border);color:var(--text2);padding:7px 14px;border-radius:14px;font-family:var(--font-body);font-size:12px;font-weight:500;cursor:pointer;white-space:nowrap" class="music-tab" data-t="albums">Альбомы</div>
          <div style="flex-shrink:0;background:var(--glass-2);border:1px solid var(--glass-border);color:var(--text2);padding:7px 14px;border-radius:14px;font-family:var(--font-body);font-size:12px;font-weight:500;cursor:pointer;white-space:nowrap" class="music-tab" data-t="artists">Исполнители</div>
          <div style="flex-shrink:0;background:var(--glass-2);border:1px solid var(--glass-border);color:var(--text2);padding:7px 14px;border-radius:14px;font-family:var(--font-body);font-size:12px;font-weight:500;cursor:pointer;white-space:nowrap" class="music-tab" data-t="playlists">Плейлисты</div>
        </div>
        ...
        <div class="music-player" id="musicPlayer" style="display:none;flex-shrink:0;margin:0 10px 10px;padding:10px;background:var(--glass-2);backdrop-filter:blur(var(--blur));-webkit-backdrop-filter:blur(var(--blur));border:1px solid var(--glass-border);border-radius:var(--rad);flex-direction:column;gap:8px">
          ...
          <div style="display:flex;align-items:center;gap:8px;justify-content:space-between">
            ...
            <span style="flex:1;display:flex;align-items:center;gap:6px;min-width:0"><span id="mpVolIcon" style="font-size:14px">🔊</span><input type="range" min="0" max="100" value="${Math.round(savedVolume*100)}" id="mpVol" style="flex:1;-webkit-appearance:none;appearance:none;height:4px;border-radius:2px;background:var(--glass-2);outline:none;min-width:0;cursor:pointer" /></span>
      ...
      if (!$('musicStyles')) {
        ...
        st.textContent = `
          .music-tab.active{background:linear-gradient(135deg,var(--accent),var(--accent-2));color:var(--on-accent);border-color:transparent;font-weight:600}
          ...
          .music-ctrl:active{transform:scale(.9)}
          ...
          .music-item{display:flex;align-items:center;gap:8px;padding:10px 12px;background:var(--glass);border:1px solid var(--glass-border);border-radius:var(--rad);margin:0 10px 6px}
          ...
          .music-item-btn:active{background:var(--glass-3);transform:scale(.9)}
  ...
  function renderSongsList(songs, header, fromPlaylist) {
    ...
    const queue = songs.map(s => ({ id: s.id, title: s.title, artist: s.artist, album: s.album, fileId: s.fileId, filename: s.filename }));
    let html = '';
  ...
  function bindPlayer() {
    ...
    function progressFromEvent(e) { const rect = prog.getBoundingClientRect(); const x = (e.touches && e.touches[0]) ? e.touches[0].clientX : (e.changedTouches && e.changedTouches[0] ? e.changedTouches[0].clientX : e.clientX); return Math.max(0, Math.min(1, (x - rect.left) / rect.width)); }
    prog.addEventListener('click', (e) => { if (!audio.duration) return; audio.currentTime = progressFromEvent(e) * audio.duration; });
    prog.addEventListener('touchstart', (e) => { if (!audio.duration) return; progressDragging = true; e.preventDefault(); const pct = progressFromEvent(e); audio.currentTime = pct * audio.duration; $('mpProgressFill').style.width = (pct * 100) + '%'; $('mpTimeCur').textContent = fmtTime(audio.currentTime); }, { passive: false });
    prog.addEventListener('touchmove', (e) => { if (!progressDragging || !audio.duration) return; e.preventDefault(); const pct = progressFromEvent(e); audio.currentTime = pct * audio.duration; $('mpProgressFill').style.width = (pct * 100) + '%'; $('mpTimeCur').textContent = fmtTime(audio.currentTime); }, { passive: false });
